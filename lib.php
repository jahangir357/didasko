...
function theme_didasko_get_pre_scss($theme) {
...
// Note the function name starts with the component name because this is a global function and we don't want namespace clashes.
function theme_didasko_get_extra_scss($theme) {
    // Load the settings from the parent.                                                                                           
    $theme = theme_config::load('boost');                                                                                           
...
function theme_didasko_get_main_scss_content($theme) {                                                                                
    global $CFG;                                                                                                                    
    ...
    $pre = file_get_contents($CFG->dirroot . '/theme/didasko/scss/pre.scss');                                                         
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.                                    
    //$post = file_get_contents($CFG->dirroot . '/theme/didasko/scss/post.scss'); 
// Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.                                    
    $post = file_get_contents($CFG->dirroot . '/theme/didasko/stylesheets/screen.css'); 	
...
function theme_didasko_update_settings_images($settingname) {                                                                         
    global $CFG;                                                                                                                    
 
    // The setting name that was updated comes as a string like 's_theme_didasko_loginbackgroundimage'.                               
    ...
    if ($file = $fs->get_file_by_hash(sha1($fullpath))) {                                                                           
        ...
        $pathname = $CFG->dataroot . '/pix_plugins/theme/didasko/' . $settingname . '.' . $extension;                                 
 
        // This pattern matches any previous files with maybe different file extensions.                                            
        $pathpattern = $CFG->dataroot . '/pix_plugins/theme/didasko/' . $settingname . '.*';                                          
 
        // Make sure this dir exists.                                                                                               
        @mkdir($CFG->dataroot . '/pix_plugins/theme/didasko/', $CFG->directorypermissions, true);                                      
